...
/**
 * Available plugins per type. <p><br /></p> > **Note**: Only unmanaged (V1) plugins are included in
 * this list. > V1 plugins are \"lazily\" loaded, and are not returned in this list > if there is no
 * resource using the plugin.
 */
case class PluginsInfo(
    /* Names of available volume-drivers, and network-driver plugins. */
    volume: Option[Seq[String]] = None,
    /* Names of available network-drivers, and network-driver plugins. */
    network: Option[Seq[String]] = None,
    /* Names of available authorization plugins. */
    authorization: Option[Seq[String]] = None,
