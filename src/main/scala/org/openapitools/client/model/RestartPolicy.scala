...
/**
 * The behavior to apply when the container exits. The default is not to restart. An ever increasing
 * delay (double the previous delay, starting at 100ms) is added before each restart to prevent
 * flooding the server.
 */
case class RestartPolicy(
    ...
    name: Option[RestartPolicyEnums.Name] = None,
    /* If `on-failure` is used, the number of times to retry before giving up.  */
    maximumRetryCount: Option[Int] = None)

object RestartPolicyEnums:
  ...
  object Name extends Enumeration:
    ...
    val UnlessStopped = Value("unless-stopped")
    val OnFailure     = Value("on-failure")

  end Name

end RestartPolicyEnums
