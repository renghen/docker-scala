...
case class EventMessage(
    ...
    `type`: Option[EventMessageEnums.`Type`] = None,
    /* The type of event */
    action: Option[String] = None,
    actor: Option[EventActor] = None,
    /* Scope of the event. Engine events are `local` scope. Cluster (Swarm) events are `swarm` scope.  */
    scope: Option[EventMessageEnums.Scope] = None,
    /* Timestamp of event */
    time: Option[Long] = None,
    /* Timestamp of event, with nanosecond accuracy */
    timeNano: Option[Long] = None)

object EventMessageEnums:
