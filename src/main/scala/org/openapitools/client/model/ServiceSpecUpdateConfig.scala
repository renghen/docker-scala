 ...
 */
case class ServiceSpecUpdateConfig(
    /* Maximum number of tasks to be updated in one iteration (0 means unlimited parallelism).  */
    parallelism: Option[Long] = None,
    /* Amount of time between updates, in nanoseconds. */
    delay: Option[Long] = None,
    /* Action to take if an updated task fails to run, or stops running during the update.  */
    failureAction: Option[ServiceSpecUpdateConfigEnums.FailureAction] = None,
    /* Amount of time to monitor each updated task for failures, in nanoseconds.  */
    monitor: Option[Long] = None,
    /* The fraction of tasks that may fail during an update before the failure action is invoked, specified as a floating point number between 0 and 1.  */
    maxFailureRatio: Option[Double] = None,
    /* The order of operations when rolling out an updated task. Either the old task is shut down before the new task is started, or the new task is started before the old task is shut down.  */
    order: Option[ServiceSpecUpdateConfigEnums.Order] = None)

object ServiceSpecUpdateConfigEnums:

  type FailureAction = FailureAction.Value
  type Order         = Order.Value

  object FailureAction extends Enumeration:
    val Continue = Value("continue")
    val Pause    = Value("pause")
    val Rollback = Value("rollback")

  end FailureAction

  object Order extends Enumeration:
    val StopFirst  = Value("stop-first")
    val StartFirst = Value("start-first")

  end Order

end ServiceSpecUpdateConfigEnums
