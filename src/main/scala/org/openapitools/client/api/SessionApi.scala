...
object SessionApi:
  def apply(baseUrl: String = "http://localhost/v1.43") = new SessionApi(baseUrl)

end SessionApi

class SessionApi(baseUrl: String):

  /**
   * Start a new interactive session with a server. Session allows server to call back to the client
   * for advanced capabilities. ### Hijacking This endpoint hijacks the HTTP connection to HTTP2
   * transport that allows the client to expose gPRC services on that connection. For example, the
   * client sends this request to upgrade the connection: ``` POST /session HTTP/1.1 Upgrade: h2c
   * Connection: Upgrade ``` The Docker daemon responds with a `101 UPGRADED` response follow with
   * the raw stream: ``` HTTP/1.1 101 UPGRADED Connection: Upgrade Upgrade: h2c ```
   *
   * Expected answers: code 101 : (no error, hijacking successful) code 400 : ErrorResponse (bad
   * parameter) code 500 : ErrorResponse (server error)
   */
  def session(): Request[Either[ResponseException[String, Exception], Unit]] =
    basicRequest
      .method(Method.POST, uri"$baseUrl/session")
