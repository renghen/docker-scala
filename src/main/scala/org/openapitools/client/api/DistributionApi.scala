...
import org.openapitools.client.model.DistributionInspect
import org.openapitools.client.model.ErrorResponse
import org.openapitools.client.core.JsonSupport.{*, given}
import sttp.client4.*
import sttp.model.Method

object DistributionApi:
  def apply(baseUrl: String = "http://localhost/v1.43") = new DistributionApi(baseUrl)

end DistributionApi

class DistributionApi(baseUrl: String):

  /**
   * Return image digest and platform information by contacting the registry.
   *
   * Expected answers: code 200 : DistributionInspect (descriptor and platform information) code 401
   * : ErrorResponse (Failed authentication or no image found) code 500 : ErrorResponse (Server
   * error)
   *
   * @param name
   *   Image name or id
   */
  def distributionInspect(name: String)
      : Request[Either[ResponseException[String, Exception], DistributionInspect]] =
    basicRequest
      .method(Method.GET, uri"$baseUrl/distribution/${name}/json")
      .contentType("application/json")
      .response(asJson[DistributionInspect])
